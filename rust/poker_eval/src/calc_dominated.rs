...
use poker_eval::{
    init_logger,
    pre_calc::{fast_eval::fast_hand_eval, perfect_hash::load_boomperfect_hash, rank::RankEnum},
    Board, BoolRange, Card, CardValue, CardValueRange, Deck, HoleCards, PokerError, Suit,
    NUM_RANK_FAMILIES,
};
...
fn main_impl() -> Result<(), PokerError> {
    ...
    let range_to_print: BoolRange =
        "A2s, K2s, Q2s, J2s, T2s, 92s, 82s, 72s, 62s, 52s, 43s".parse()?;
        ...
        for col in 0..13 {
            ...
            for _ in 0..num_hands_to_simulate {
                ...
                let is_above_threshold = rankings
                    .iter()
                    .map(|r| r >= &min_rank[hi_card as usize])
                    .collect_vec();
            ...
            dv.push((
                "% 2 or 3 with good hands".to_string(),
                sum_heads_up as f64 / num_hands_to_simulate as f64,
            ));
            dv.push((
                "% beat second best HU".to_string(),
                beat_second_best as f64 / sum_heads_up as f64,
            ));
            dv.push((
                "% two way tie HU".to_string(),
                two_way_tie as f64 / sum_heads_up as f64,
            ));
            dv.push((
                "% was second best HU".to_string(),
                was_second_best as f64 / sum_heads_up as f64,
            ));

            dv.push((
                "% was same family".to_string(),
                was_same_family as f64 / num_hands_to_simulate as f64,
            ));
            ...
            for fs in rank_families_to_print.iter() {
                ...
                dv.push((
                    format!("% tie {}", fs),
                    tie_family_stats[fs_index] as f64 / same_family_stats[fs_index] as f64,
                ));
                dv.push((
                    format!("% lost {}", fs),
                    lose_family_stats[fs_index] as f64 / same_family_stats[fs_index] as f64,
                ));
            }
