...
//AgentRunner -- Runs a games w/ agents

//GameRunner -- Runs partial game log, action by action, updates GameState
// and validates actions
...
impl<'a> AgentRunner<'a> {
    fn new(agents: Vec<Box<dyn Agent + 'a>>, small_blind: ChipType) -> AgentRunner {
        ...
        AgentRunner {
            game_state: OldGameState::new(agents.len() as u8),
    ...
    fn init_agent_pos_and_stack(&mut self, position: Position, stack: ChipType) {
        let agent_state: &mut AgentState =
            &mut self.agent_states[usize::try_from(position).unwrap()];
    ...
    fn run_round(&mut self, round: Round) {
        ...
        if round == Round::Preflop {
            ...
            self.game_state.current_pot +=
                self.agent_states[usize::from(BIG_BLIND)].handle_put_money_in_pot(self.big_blind);
        }
...
mod tests {
    ...
    fn test_split_pots() {
        ...
        let mut game_runner = AgentRunner::new(agents, 1);
